## Feature Overview
In the 3.3.0 release additional block device mapping support was added for EBS backed instances. Block device mapping and ebs backed instances are not new to Eucalyptus. Eucalyptus - 3.3.0 has added support for multiple block devices as well as addresses multiple user requests for additional AWS compliance and attribute consistency.  EBS backed instances are often referred to as 'boot from EBS' (bfebs) instances, or instances with an EBS root device.
