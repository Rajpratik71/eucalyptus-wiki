...
##### Block Device Mapping attributes and examples
A block device mapping can determine the name of the block devices (ie: /dev/sda), size of the device (in GB), type of device (ephemeral or ebs), source of the device (ie snapshot to create the device from), and whether or not to delete the EBS volume backing the device. 
